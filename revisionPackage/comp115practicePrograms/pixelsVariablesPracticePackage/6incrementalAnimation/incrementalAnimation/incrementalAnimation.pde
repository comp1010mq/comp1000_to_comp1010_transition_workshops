...
void mousePressed() {
  ...
  thickness = map(thickness, 0, 500, 1, 10);
  strokeWeight(thickness);
  line(previousPressX, previousPressY, mouseX, mouseY);
  //update previous press location for next time
  previousPressX = mouseX;
  previousPressY = mouseY;
}